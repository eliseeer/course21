...
#    while True:
 #       j = math.sin(i)/10
  #      h.set_pos(h.get_pos().x,h.get_pos().y,h.get_pos().z+j)
   #     i += 1
    #    time.sleep(0.1)
...
def test_ambf_ecm():
    ...
    h = c.get_obj_handle('ecm/Heart')
    target_ik = c.get_obj_handle('ecm/target_ik')
    target_fk = c.get_obj_handle('ecm/target_fk')
    time.sleep(1.0)

    joint_pub = rospy.Publisher("/ambf/env/ecm/baselink/Command", RigidBodyCmd, tcp_nodelay=True, queue_size=10)
